...
-- 기존의 직업을 여전히 가지고있는 사원들의 사번 및 job__id 조회 
-- employees, job_history이용
...
-- JOB_ID가 SA_MAN 인 사원들의 최대연봉보다 높게 받는 사원들의 LAST_NAME, JOB_ID, SALARY 조회
SELECT
	LAST_NAME,
	JOB_ID,
	SALARY
...
-- 커미션을 버는 사원들의 부서와 연봉이 동일한 사원들의 LAST_NAME, 무서번호, salary 조회
SELECT
	LAST_NAME,
	DEPARTMENT_ID,
	SALARY
...
-- 회사 전체 평균 연봉보다 많이 버는 사원들 중 LAST_NAME에 u가 없는 사원들이 근무하는 부서에서 근무하는 사원들의 사번, LAST_NAME, 연봉 조회
SELECT
	EMPLOYEE_ID,
	LAST_NAME,
	SALARY
...
FROM EMPLOYEES e
WHERE LAST_NAME LIKE '%u%' AND SALARY > (SELECT AVG(SALARY) FROM EMPLOYEES e2);
...
WHERE
	...
	AND E1.SALARY > E2.DEPT_SAL_AVG
ORDER BY
	E1.DEPARTMENT_ID; 
		

-- LAST_NAME이 davies 인 사람보다 후에 고용된 사원들의 LAST_NAME, hire_date조회
...
-- LAST_NAME이 king인 사원을 매니저로 두고있는 모든 사람들의 LAST_NAME, salary 조회
SELECT LAST_NAME, SALARY 
FROM EMPLOYEES e 
...
-- countries 테이블과 ;ocations, departments 이용

-- 총사원 수 및 2013, 2014, 2015, 2016년도 별 고용된 사람들의 총 수를 조회한다.
-- SELECT COUNT(EMPLOYEE_ID) FROM EMPLOYEES WHERE TO_CHAR(HIRE_DATE, 'YY') = '13'
-- SELECT COUNT(EMPLOYEE_ID) FROM EMPLOYEES WHERE HIRE_DATE LIKE '13%'
...
-- 위치 ID 가 1700인 사원들의 연봉과 커미션을 추출한 뒤, 추출된 사원들의 연봉과 커미션이 동일한 사원번호 조회
-- 사원번호,이름,부서번호 급여 출력
